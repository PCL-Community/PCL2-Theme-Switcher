...
def get_pcl_path():
    ...
    if not os.path.exists(PCL_dir_path):
        print(
            "这里好像没有 PCL 文件夹，请确保程序的位置和PCL exe在同一目录下且有 PCL 文件夹。"
        )
        input("按任意键后回车退出。")
        exit()
...
def modify_logs():
    def Log4():  # 将 Log4 弄到 Log5
        if os.path.exists(PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log4.log"):
            with open(
                PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log4.log",
                "r",
                encoding="utf-8",
            ) as log4:
                log4_data = log4.read()  # 存储为
            with open(
                PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log5.log",
                "w",
                encoding="utf-8",
            ) as log5:
    ...
    def Log3():  # 将 Log3 弄到 Log4
        if os.path.exists(PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log3.log"):
            with open(
                PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log3.log",
                "r",
                encoding="utf-8",
            ) as log3:
    ...
    def Log2():  # 将 Log2 弄到 Log3
        if os.path.exists(PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log2.log"):
            with open(
                PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log2.log",
                "r",
                encoding="utf-8",
            ) as log2:
    ...
    def Log1():  # 将 Log1 弄到 Log2
        if os.path.exists(PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log1.log"):
            with open(
                PCL_exe_path + "/PCL2 Theme Switcher/Logs/Log1.log",
                "r",
                encoding="utf-8",
            ) as log1:
...
modify_logs()
logging.basicConfig(
    filename=f"{PCL_exe_path}/PCL2 Theme Switcher/Logs/Log1.log",
    level=logging.INFO,
...
def backup_pcl_config():
    ...
    if os.path.exists(PCL_dir_path):  # 检测是否存在 "PCL" 文件夹
        ...
        if os.path.exists(PCL_dir_path + "/setup.ini"):  # 检测是否存在 "setup.ini"
            logging.info(
                rf"[Backup] setup.ini 文件存在。位置为： {PCL_dir_path}\setup.ini"
            )  # logging - setup.ini
            print("已找到 PCL 配置文件。")  # print - setup.ini
            logging.info(
                rf"[Backup] 正在备份 setup.ini 配置文件。备份后的文件位置为： {PCL_exe_path}\PCL2 Theme Switcher\setup.ini"
            )  # logging - backing up setup.ini
            print("正在备份 PCL 配置文件。")  # print - backing up setup.ini
            with open(
                PCL_dir_path + "/setup.ini", "r", encoding="utf-8"
            ) as setup_ini:  # 读 setup.ini 内容
                setup_ini_data = setup_ini.read()  # setup_ini_data 是 setup.ini 的内容
                logging.info("[Backup] 读取 setup.ini 完毕")
            with open(
                f"{PCL_exe_path}/PCL2 Theme Switcher/setup.ini", "w", encoding="utf-8"
            ) as backup_setup_ini_data:
                backup_setup_ini_data.write(setup_ini_data)
                logging.info(
                    rf"[Backup] setup.ini 已备份。备份到： {PCL_exe_path}\PCL2 Theme Switcher\setup.ini"
                )
                print("setup.ini 已备份。")
        else:
            ...
            print("\n")
            print(
                "\033[1m没有找到位于 PCL 文件夹下的 setup.ini 文件。请确认你的 setup.ini 是否存在。\033[0m"
            )
            exit()
    else:
...
def get_theme_color():
    ...
    if os.path.exists(PCL_dir_path):  # 检测是否存在 "PCL" 文件夹
        ...
        if os.path.exists(PCL_dir_path + "/setup.ini"):  # 检测是否存在 "setup.ini"
            logging.info(
                rf"[Get Theme Color] setup.ini 文件存在。位置为： {PCL_dir_path}\setup.ini"
            )  # logging - setup.ini
            with open(
                PCL_dir_path + "/setup.ini", "r", encoding="utf-8"
            ) as setup_ini:  # 读 setup.ini 内容
    ...
    def extract_value(data, key):
        ...
        for line in data.splitlines():
            ...
            mid_key = key
        logging.warning(
            "[Get Theme Color] " + key + ": 没有那个数据。"
        )  # Designed by Ubuntu
        key = "error"
        return key  # 没有找到数据时返回 "error"，供下面判断使用

    """开始获取主题值"""
    UiLauncherTheme = extract_value(
        data, "UiLauncherTheme"
    )  # 获取当前主题类型，14为自定义主题
    UiLauncherTransparent = extract_value(
        data, "UiLauncherTransparent"
    )  # 获取当前透明度，0为40%不透明，600为100%不透明
    UiLauncherDelta = extract_value(
        data, "UiLauncherDelta"
    )  # 获取当前主题色调渐变，0为-90色调渐变，180为+180色调渐变
    UiLauncherSat = extract_value(
        data, "UiLauncherSat"
    )  # 获取当前主题饱和度，0为0%饱和度，100为100%饱和度
    UiLauncherHue = extract_value(
        data, "UiLauncherHue"
    )  # 获取当前主题色调，0为0°色调，360为360°色调
    UiLauncherLight = extract_value(
        data, "UiLauncherLight"
    )  # 获取当前主题亮度，0为-20亮度，40为+20亮度
...
def tech_otakus_save_the_pcl_theme_color():  # 技术宅保存PCL主题颜色
    ...
    logging.info(
        "[Save Theme Color] 正在保存主题颜色，文件名： " + save_file_name + ".ini 。"
    )
    if os.path.exists("PCL2 Theme Switcher/saves/" + save_file_name + ".ini"):
        while True:
            logging.info(
                "[Save Theme Color] 文件名已存在，询问是否重新键入或覆盖文件。"
            )
            warning_file = input(
                "\033[1m文件名已存在，请重新键入，或按 Enter 覆盖文件。\033[0m\n保存的文件名："
            )
            if warning_file == "":
                ...
                break
            if (
                warning_file == save_file_name
            ):  # 这是警告后输入的和原本输的还是一样的文件名，那只能说你太闲了！
                continue  # 干啥的？不到啊！
            else:  # 这是如果你警告后输对其他的文件名，别搞混咯~
                save_file_name = warning_file  # 保存文件名
                break  # 退出循环
    with open(
        "PCL2 Theme Switcher/saves/" + save_file_name + ".ini", "w", encoding="utf-8"
    ) as save:
        save.write("UiLauncherTransparent:" + UiLauncherTransparent + "\n")
        logging.info(
            "[Save Theme Color] 已写入 UiLauncherTransparent:" + UiLauncherTransparent
        )
        print("\n")
...
def replace_pcl_theme_color():  # 技术宅替换PCL主题颜色
    ...
    files = os.listdir(
        PCL_exe_path + "/PCL2 Theme Switcher/saves"
    )  # 获取当前目录下的所有文件
    ...
    while True:
        ini_file = input(
            "\033[1m键入你想替换的文件名，目前仅支持打开此目录下 PCL2 Theme Switcher/saves 里面的配置文件，无需输入.ini\033[0m:  "
        )
        if os.path.exists(
            PCL_exe_path + "/PCL2 Theme Switcher/saves/" + ini_file + ".ini"
        ):
            logging.info("[Replace] 即将从此处替换 setup.ini： " + ini_file + ".ini")
            with open(
                "PCL2 Theme Switcher/saves/" + ini_file + ".ini", "r", encoding="utf-8"
            ) as ready_replace:
            ...
            logging.info(
                f"[Replace] 已从 {ini_file}.ini 写入 UiLauncherTransparent(透明度):"
            ...
            logging.info(
                f"[Replace] 已从 {ini_file}.ini 写入 UiLauncherDelta(色调渐变):"
                + UiLauncherDelta
            )
            ...
            logging.info(
                f"[Replace] 已从 {ini_file}.ini 写入 UiLauncherSat(饱和度):"
                + UiLauncherSat
            )
            ...
            logging.info(
                f"[Replace] 已从 {ini_file}.ini 写入 UiLauncherHue(色调):"
                + UiLauncherHue
            )
            ...
            logging.info(
                f"[Replace] 已从 {ini_file}.ini 写入 UiLauncherLight(亮度):"
                + UiLauncherLight
            )
            ...
            with open(
                ...
                logging.info(
                    r"[Replace] 现在的 PCL\setup.ini 的内容为：" + "\n" + temp_review
                )
            break
        else:
            logging.error(f"请求的文件 {ini_file}.ini 不存在，已请求用户重新选择。")
            print(
                f"文件 \033[1m{ini_file}.ini\033[0m 不存在，请重新输入\033[1m（无需带.ini）\033[0m 。"
            )
